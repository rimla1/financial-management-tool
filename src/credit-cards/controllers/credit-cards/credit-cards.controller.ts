...
export class CreditCardsController {
    ...
    getCreditCards(){
        try {
            const creditCards = this.creditCardsService.getCreditCards()
            return creditCards 
        } catch (e) {
            console.log(e)
        }
    ...
    createCreditCard(@Body() creditCardInfo: CreateCreditCardDto){
        try {
            const newCreditCard = this.creditCardsService.createCreditCard(creditCardInfo)
            return "name, balance, createdAt, updatedAt"
        } catch (e) {
            console.log(e)
        }
    ...
    @Patch(":id")
    updateCreditCard(@Body() creditCardInfo: UpdateCreditCardDto, @Param("id", ParseIntPipe) id: number){
        try {
            const updatedCreditCard = this.creditCardsService.updateCreditCard(id, creditCardInfo)
